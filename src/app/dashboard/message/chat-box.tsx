...
function ChatBox(): JSX.Element {
  ...
  useEffect(() => {
    ...
    ablyChannel.subscribe('chat-message', onMessage);
    
    return () => {
      ablyChannel.unsubscribe('chat-message', onMessage);
    };
  }, [ablyChannel, currentGroup]);
  ...
  const sendChatMessage = async (messageText: string) => {
    try {
      if (currentGroup && ablyChannel && user_) {
        ...
        await ablyChannel.publish("chat-message", message);
